<script lang="ts">
    ...
    function savedLeague(): LeagueId {
        ...
        return LEAGUES.some((l) => l.id === stored)
            ? (stored as LeagueId)
            : "nhl";
    }
...
<div class="min-h-screen bg-gray-950 text-white">
    <header class="py-10 text-center">
        <h1 class="font-condensed text-6xl font-black tracking-tight">
            Numbrrs
        </h1>
        <p class="mt-2 text-lg text-gray-400">
            Learn hockey jersey numbers, one card at a time
        </p>
    </header>
    ...
    <main class="mx-auto max-w-5xl px-4 pb-12">
            ...
            >
                {#each LEAGUES as option (option.id)}
                    <button
                        ...
                        class="font-condensed rounded-md px-6 py-1.5 text-sm font-bold uppercase tracking-widest transition-colors {league ===
                        option.id
        ...
        <h2
            class="font-condensed mb-6 text-center text-2xl font-bold uppercase tracking-widest text-gray-400"
        >
            Choose a Team
        </h2>
        ...
        {:else if leagueTeams.length === 0}
            <p class="text-center text-gray-400">
                Loading teams... Roster data is syncing from the league API.
                Refresh in a moment.
            </p>
        {:else}
            <div
            ...
            >
                {#each leagueTeams as team (team.id)}
                    ...
                    >
                        ...
                        />
                        <span
                            class="font-condensed text-center text-sm font-bold uppercase tracking-wide text-gray-300"
                        >
                            {team.name}
                        </span>
                    </a>
